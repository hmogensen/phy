...
class Supervisor(object):
    ...
    * `save_clustering(spike_clusters, cluster_groups, *cluster_labels)`
    ...
    def __init__(
        ...
        self.triggers = triggers or {}  # Dictionary of trigger_name -> array of times
    ...
    def get_trigger_info(self, trigger_id, trigger_name):
        """Return the data associated to a given trigger."""
        times = self.triggers.get(trigger_name, [])
        return {
            'id': trigger_id,
            'name': trigger_name,
    ...
    def _create_views(self, gui=None, sort=None):
        ...
        self.trigger_view = TriggerView(
            gui,
            data=[self.get_trigger_info(i, name) for i, name in enumerate(self.triggers)],
            columns=['id', 'name', 'n_triggers'],
            sort=('name', 'asc')
        )
