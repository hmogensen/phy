...
class BaseController(object):
    ...
    def _set_view_creator(self):
        ...
        self.view_creator = {
            ...
            'PeristimHistView': self._make_histogram_view(PeristimHistView, self._get_peristim_hist),
    ...
    def _get_trigger_times(self, trigger_name):
        """Return trigger times for selected trigger"""
        return self.model.trigger_times[self.model.trigger_groups == trigger_name]
    ...
    def _get_peristim_hist(self, cluster_id):
        ...
        if trigger_ids is None:
            return Bunch(data=np.array([]), x_min=window[0], x_max=window[1])
        
        # Collect trigger times from all selected triggers
        trigger_times = []
        for trigger_id in trigger_ids:
            # Get trigger times from triggers dict using trigger ID
            trigger_times.extend(self.model.triggers.get(trigger_id, []))
